...
const WORLD_LAYER_LABELS = { surface: '表世界', inner: '里世界', transition: '世界转换中', unknown: '未说明' }
    ...
    <div className="developer-workbench-layout">
      <section className="developer-page-workspace">
        ...
        {!scene ? <div className="developer-no-page"><p>本章还没有 Scene。</p><button type="button" onClick={addScene}>新建 Scene 01</button></div> : <>
          <section className="developer-current-page-info"><h2>当前 Scene</h2><p><code>{scene.id}</code> · 第 {scene.order} 个 Scene</p><div className="developer-environment-fields" aria-label="Scene 场景状态"><label>世界<select value={scene.context.worldLayer} onChange={event => updateSceneContext({ worldLayer: event.target.value })}>{Object.entries(WORLD_LAYER_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label>地点 ID<input value={scene.context.locationId} onChange={event => updateSceneContext({ locationId: event.target.value })} placeholder="ji_ancestral_residence_courtyard" /></label><label>{LANGUAGE_LABELS[editingLanguage]}地点显示名<input value={scene.context.locationLabels[editingLanguage]} onChange={event => updateLocationLabel(event.target.value)} placeholder="姬家祖宅外院" /></label><label>时间<input value={scene.context.time} onChange={event => updateSceneContext({ time: event.target.value })} placeholder="daylight" /></label><label>天气<input value={scene.context.weather} onChange={event => updateSceneContext({ weather: event.target.value })} placeholder="clear" /></label></div><small>这些字段只描述 Scene 的稳定状态；不会把 Reader 切页。缺少资料时保留“未说明 / unknown”，不要补写设定。Scene 的边界本身由你确定，不额外保存事件标记。</small><div className="developer-page-tabs" aria-label="Scene 正文语言">{SCENE_LANGUAGES.map(language => <button type="button" key={language} aria-pressed={editingLanguage === language} onClick={() => setEditingLanguage(language)}>{LANGUAGE_LABELS[language]}</button>)}</div><textarea className="developer-page-editor" aria-label={`${LANGUAGE_LABELS[editingLanguage]} Scene 正文`} spellCheck="false" value={scene.content[editingLanguage]} onChange={event => updateSceneText(event.target.value)} placeholder={`${LANGUAGE_LABELS[editingLanguage]} 正文`} /><div className="developer-page-tools"><button type="button" onClick={addScene}>在后面加入下一个 Scene</button><button type="button" onClick={removeScene}>移除当前 Scene</button></div></section>
        </>}
