    ...
    <div className="developer-workbench-layout">
      <section className="developer-page-workspace">
        ...
        {!scene ? <div className="developer-no-page"><p>本章还没有 Scene。</p><button type="button" onClick={addScene}>新建 Scene 01</button></div> : <>
          <section className="developer-current-page-info"><h2>当前 Scene</h2><p><code>{scene.id}</code> · 第 {scene.order} 个 Scene</p><div className="developer-page-tabs" aria-label="Scene 正文语言">{SCENE_LANGUAGES.map(language => <button type="button" key={language} aria-pressed={editingLanguage === language} onClick={() => setEditingLanguage(language)}>{LANGUAGE_LABELS[language]}</button>)}</div><textarea className="developer-page-editor" aria-label={`${LANGUAGE_LABELS[editingLanguage]} Scene 正文`} spellCheck="false" value={scene.content[editingLanguage]} onChange={event => updateSceneText(event.target.value)} placeholder={`${LANGUAGE_LABELS[editingLanguage]} 正文`} /><div className="developer-page-tools"><button type="button" onClick={addScene}>在后面加入下一个 Scene</button><button type="button" onClick={removeScene}>移除当前 Scene</button></div></section>
        </>}
