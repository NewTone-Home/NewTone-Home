...
export function sanitizeProgress(value) {
  ...
  return { ...createInitialProgressState(), currentView: source.currentView === 'reader' ? 'reader' : 'landing', committedLocation, furthestLocation, readerStarted: Boolean(source.readerStarted), readerCompleted: Boolean(source.readerCompleted), resumeRequested: Boolean(source.resumeRequested), readerExitGestureLearned: Boolean(source.readerExitGestureLearned ?? source.exitTutorialSeen), chapterTrialEnded: source.chapterTrialEnded === true, language: READER_LANGUAGE_CODES.includes(source.language) ? source.language : 'zh', hasInitializedLanguage: Boolean(source.hasInitializedLanguage), hasInitializedReadingMode: Boolean(source.hasInitializedReadingMode ?? source.hasInitializedLanguage), readingMode: 'standard', themePosition, standardTheme: legacyThemeName(themePosition), motionMode: ['full', 'reduced'].includes(source.motionMode) ? source.motionMode : 'full', currentChapter: chapterAt(committedLocation), currentPage: committedLocation.pageId, currentBeat: committedLocation.beatIndex, readerScrollOffset: Number.isFinite(source.readerScrollOffset) ? Math.max(0, source.readerScrollOffset) : 0 }
}
