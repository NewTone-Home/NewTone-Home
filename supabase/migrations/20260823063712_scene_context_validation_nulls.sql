-- PostgreSQL JSON operators yield NULL for missing keys, so use IS DISTINCT FROM
-- for every structural check that must reject an absent value.
  ...
  for chapter in select item from jsonb_array_elements(value->'chapters') as item loop
    ...
    for scene in select item from jsonb_array_elements(chapter->'scenes') as item loop
      ...
      if schema_version = '3' then
        ...
        if jsonb_typeof(scene_context->'eventIds') is distinct from 'array' then return false; end if;
        for event_id in select item from jsonb_array_elements(scene_context->'eventIds') as item loop
          if jsonb_typeof(event_id) is distinct from 'string' or nullif(btrim(event_id #>> '{}'), '') is null then return false; end if;
        end loop;
      end if;
