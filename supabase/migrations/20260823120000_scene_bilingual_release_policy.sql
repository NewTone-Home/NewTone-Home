-- The public Reader currently supports Chinese and English only.
-- Keep the canonical Scene structure unchanged; only align the release gate
-- with the languages that the product actually exposes.
...
declare
  ...
  required_languages text[] := ARRAY['zh', 'en'];
...
begin
  ...
  if jsonb_typeof(value->'languages') is distinct from 'array' or value->'languages' is distinct from '["zh","en"]'::jsonb then return false; end if;
