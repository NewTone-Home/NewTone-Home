...
describe('shared entry button surface', () => {
  it('owns one explicit visual timeline and click completion boundary', () => {
    ...
    expect(surface).toContain("import { ENTRY_BUTTON_TIMINGS, createEntryProgress, useEntryButtonTimeline } from './entryButtonTimeline'")
    expect(surface).not.toContain('const TIMINGS =')
    expect(surface).not.toContain('const runTimeline =')
  ...
  it('clips fill and frame to the same SVG path', () => {
    ...
    expect(frame).toContain('data-entry-fill-geometry={shapeId}')
  ...
  it('keeps material source separate from host state', () => {
    ...
    expect(frame).toContain("var(--entry-world-surface-highlight)")
    expect(frame).toContain("var(--entry-world-inner-highlight)")
    ...
    expect(styles).toContain('--entry-world-surface-body')
    expect(styles).toContain('--entry-world-surface-opacity')
    expect(styles).toContain('--entry-world-inner-body')
    expect(styles).toContain('--entry-world-inner-opacity')
