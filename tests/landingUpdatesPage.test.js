...
describe('Landing updates page button contract', () => {
  ...
  it('renders the selected announcement layout and copy', () => {
    ...
    expect(page).toContain('合并了阅读模式，现在不再分成两个模式。')
    expect(page).toContain('优化了阅读体验。')
    expect(page).toContain('优化了操作体验。')
    expect(page).toContain('The reading modes are now combined into one, so there are no longer two separate modes.')
    expect(page).toContain('The reading experience has been improved.')
    expect(page).toContain('The interaction experience has been improved.')
