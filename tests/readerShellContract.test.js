...
const readerReturnFlow = read('../src/reader/readerReturnFlow.js')
...
describe('Reader shell contract boundaries', () => {
  ...
  it('keeps the full Reader shell mounted when no publication exists', () => {
    ...
    expect(stage).toContain('const shouldMountReturnControl = returnFlow.state !== READER_RETURN_STATE.HIDDEN')
    expect(stage).toContain('const shouldShowReturnControl = shouldMountReturnControl')
    expect(stage).toContain('createReaderReturnFlowState')
  ...
  it('offers the return choice at the real page end and at the final node on direct-input devices', () => {
    ...
    expect(stage).toContain('returnFlow.state !== READER_RETURN_STATE.HIDDEN')
    expect(stage).toContain('shouldMountReturnControl && shouldShowReturnControl')
    expect(stage).toContain('!emptyDocument\n      && directReaderInput\n      && finalNodeReached')
    ...
    expect(stage).toContain("event.pointerType !== 'mouse'")
    expect(stage).toContain('returnEntryReady')
    expect(stage).toContain("if (event.detail !== 0) return")
    expect(stage).toContain('if (returnFlowRef.current.state === READER_RETURN_STATE.ARMED)')
    expect(returnControl).toContain('onPointerEnter?.(event)')
    expect(returnControl).toContain('onPointerLeave?.(event)')
    expect(stage).toContain('onReadyChange={setReturnEntryReady}')
    expect(stage).toContain('exitRequestId={exitRequestId}')
    expect(stage).toContain('READER_RETURN_EVENT.FORWARD_GESTURE')
    expect(stage).toContain('READER_RETURN_EVENT.REVERSE_GESTURE')
    expect(returnControl).toContain('if (!exitRequestId)')
    expect(returnControl).toContain("direction={completing ? 'left' : hovered || armed ? 'down' : 'right'}")
    expect(returnControl).toContain('data-return-progress={progress.toFixed(3)}')
    expect(stage).toContain("window.addEventListener('wheel', onWheel, { capture: true, passive: false })")
    expect(stage).toContain("window.addEventListener('pointermove', onPointerMove, { capture: true, passive: false })")
    expect(returnControl).not.toContain("window.addEventListener('wheel'")
    expect(returnControl).not.toContain("window.addEventListener('pointermove'")
    ...
    expect(readerReturnFlow).toContain("HIDDEN: 'hidden'")
    expect(readerReturnFlow).toContain("REVEALING: 'revealing'")
    expect(readerReturnFlow).toContain("READY: 'ready'")
    expect(readerReturnFlow).toContain("ARMED: 'armed'")
    expect(readerReturnFlow).toContain("DISMISSING: 'dismissing'")
  ...
  it('retracts the return affordance before confirmed navigation instead of hiding it in one frame', () => {
    ...
    expect(returnControl).toContain('pendingCompleteRef.current = navigate')
    ...
    expect(returnControl).toContain('handledExitRequestRef.current')
    expect(returnControl).toContain('onReadyChange?.(entryReady || completing)')
    expect(returnControl).toContain('const startExit = useCallback((navigate = true) =>')
    expect(returnControl).toContain("startExitRef.current?.(exitRequestMode === 'return')")
    expect(returnControl).not.toContain('RETURN_DIRECT_THRESHOLD')
    expect(returnControl).toContain('onDismissStartRef.current?.()')
    expect(returnControl).toContain('!dismissOnlyRef.current')
    expect(returnControl).toContain('onDismissCompleteRef.current?.()')
