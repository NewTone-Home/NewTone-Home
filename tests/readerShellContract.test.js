...
describe('Reader shell contract boundaries', () => {
  ...
  it('reverses the simple text-then-frame animation from its current progress', () => {
    expect(returnControl).toContain('const TEXT_ENTER_MS = 320')
    expect(returnControl).toContain('const FRAME_DRAW_MS = 720')
    expect(returnControl).toContain('const DOOR_OPEN_MS = 520')
    expect(returnControl).toContain('const TEXT_EXIT_MS = 260')
    expect(returnControl).toContain('const DOOR_CLOSE_MS = 520')
    expect(returnControl).toContain('const FRAME_RETRACT_MS = 420')
    expect(returnControl).toContain("animateValue('text', 0")
    expect(returnControl).toContain("animateValue('frame', 0")
    expect(returnControl).toContain('progressRef.current[key]')
    expect(returnControl).toContain('onReturnCompleteRef.current?.()')
    expect(returnControl).toContain("animateValue('door', open ? 1 : 0")
    expect(returnControl).toMatch(/animateValue\('text', 0[\s\S]*animateValue\('door', 0[\s\S]*animateValue\('frame', 0/)
    ...
    expect(returnControl).toContain('perspective(160px)')
  })
  ...
  it('draws one random-origin frame and one random-direction world-layer mask', () => {
    ...
    expect(returnControl).toContain('data-return-active={active')
    expect(returnControlCss).toContain("data-return-world-layer='inner'")
    ...
    expect(returnControlCss).toContain('reader-return-room')
  })
