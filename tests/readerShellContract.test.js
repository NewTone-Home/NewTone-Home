...
describe('Reader shell contract boundaries', () => {
  ...
  it('keeps the full Reader shell mounted when no publication exists', () => {
    ...
    expect(stage).toContain('returnVisible = !returnDismissed && (')
  ...
  it('offers the return choice at the real page end and at the final node on direct-input devices', () => {
    ...
    expect(returnControl).toContain('event.detail !== 0')
    expect(returnControl).toContain('if (armed) onDisarm()')
    expect(stage).toContain('onDisarm={() => updateReturnArmed(false)}')
    expect(stage).toContain('onDismissStart={() => {')
    expect(stage).toContain('setReturnExiting(true)')
    expect(stage).toContain('onDismissComplete={() => {')
    ...
    expect(beatStack).toContain('if (returnArmed) {')
    expect(beatStack).toContain('if (!atBottomRef.current || event.deltaY <= RETURN_WHEEL_THRESHOLD) return')
    expect(beatStack).toContain('if (event.cancelable) event.preventDefault()')
    expect(beatStack).toContain("viewport.addEventListener('wheel', onWheel, { passive: false })")
  ...
  it('retracts the return affordance before confirmed navigation instead of hiding it in one frame', () => {
    ...
    expect(returnControl).toContain('}, [maybeCompleteReturn, onDisarm, visualArmed])')
    expect(returnControl).toContain('onReadyChange?.(visualArmed || completing)')
    expect(returnControl).toContain('if (Math.abs(gesture.startY - event.clientY) <= RETURN_DIRECT_THRESHOLD) onDisarm()')
    expect(returnControl).toContain('const startExit = (navigate = true) =>')
    expect(returnControl).toContain('startExit(false)')
    expect(returnControl).toContain('event.deltaY < -RETURN_WHEEL_THRESHOLD')
  ...
  it('arms the mobile updates return once and routes the next explicit downward swipe', () => {
    ...
    expect(updatesPage).toContain('cancelMobileReturnActivation')
    expect(updatesPage).toContain('gesture.startY - event.clientY >= 42')
    expect(updatesPage).toContain('reason: RETURN_ENTRY_WITHDRAWAL_REASON.WHEEL')
