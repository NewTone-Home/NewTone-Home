...
describe('Reader shell contract boundaries', () => {
  ...
  it('keeps Landing prompts complete and selection circles retractable', () => {
    ...
    expect(transitionCss).toContain('transition-delay: 1200ms')
    ...
    expect(transitionHoverCss).toContain('ritual-entry-ring-draw 1700ms')
    expect(transitionHoverCss).toContain('stroke-dashoffset 900ms')
  ...
  it('retracts the return affordance before confirmed navigation instead of hiding it in one frame', () => {
    expect(returnControl).toContain('const RETURN_RING_RETRACT_MS = 900')
    ...
    expect(returnControlCss).toContain('transition: opacity 320ms ease')
    expect(returnControl).toContain('const RETURN_RING_DRAW_MS = 1700')
  ...
  it('arms the mobile updates return once and routes the next explicit upward swipe', () => {
    ...
    expect(updatesPageCss).toContain('updates-return-ring-draw 1700ms')
  })
