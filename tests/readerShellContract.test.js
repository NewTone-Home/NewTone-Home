...
describe('Reader shell contract boundaries', () => {
  ...
  it('offers the return choice at the real page end and at the final node on direct-input devices', () => {
    ...
    expect(stage).toContain('onReverseGesture={() => {')
    expect(stage).toContain('onDisarmComplete={() => {')
  ...
  it('retracts the return affordance before confirmed navigation instead of hiding it in one frame', () => {
    ...
    expect(returnControl).toContain('}, [maybeCompleteReturn, onDisarm, onReverseGesture, visualArmed])')
    ...
    expect(returnControl).toContain('onReverseGesture?.()')
    expect(returnControl).toContain('onDisarmCompleteRef.current?.()')
  ...
  it('arms the mobile updates return once and routes the next explicit upward swipe', () => {
    ...
    expect(updatesPage).toContain('event.clientY - gesture.startY >= 42')
    expect(updatesPage).toContain('reason: RETURN_ENTRY_WITHDRAWAL_REASON.LEAVE')
