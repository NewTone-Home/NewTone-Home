...
describe('Reader shell contract boundaries', () => {
  ...
  it('fades the restored thin hand-drawn Landing guide in and out', () => {
    expect(landing).toContain('className="landing-title-n-host"')
    expect(landing).toContain("{'ewTone'}")
    expect(landing).toContain('viewBox="0 0 92 72"')
    ...
    expect(landing).not.toContain('GUIDE_SHAPES')
    expect(landing).not.toContain('guidePathAt')
    expect(landingCss).toContain('right: calc(100% - 20px)')
    expect(landingCss).toContain('stroke-width: 1.35')
    expect(landingCss).toContain('transition: opacity 300ms ease')
    expect(landingCss).toContain('.landing-guide-arrow--visible')
    expect(landingCss).toContain('.landing-guide-arrow--retracting')
    expect(landingCss).not.toContain('landing-guide-drift')
  ...
  it('hands Reader return into the complete interactive Landing', () => {
    ...
    expect(landing).toContain('setReturnArrivalActive(true)')
    expect(landing).toContain('window.requestAnimationFrame(() => begin())')
    expect(landing).toContain('phase === TITLE_PHASE.REVEALED) setReturnArrivalActive(false)')
    expect(landing).toContain("readerReturnHandoffActive ? ' landing--return-handoff' : ''")
    expect(landing).toContain("returnArrivalActive ? ' landing--return-arrival' : ''")
    expect(landingCss).toContain('.landing--return-handoff .landing-title-stack')
    expect(landingCss).toContain('visibility: hidden')
    expect(landingCss).toContain('.landing--return-arrival .landing-title-draft')
    expect(landingCss).not.toContain('transition: opacity 180ms ease-out')
    expect(app).toContain("tState.transitionTo('landing', { preset: 'reader-to-surface', waitForReady: false })")
    ...
    expect(globalTransitionCss).toContain('.phase-entering .global-transition-newtone .landing-title-draft')
  })
  ...
  it('offers an unarmed return choice only at the real bottom of every page', () => {
    ...
    expect(returnControl).toContain("event.pointerType !== 'mouse' && !armed")
