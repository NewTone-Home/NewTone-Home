...
describe('Reader shell contract boundaries', () => {
  ...
  it('keeps Landing prompts complete and selection circles retractable', () => {
    ...
    expect(transition).toContain('<RitualHandAffordance />')
    expect(transition).toContain('<LanguageExpandAffordance onAnimationEnd={handleLanguageAffordanceAnimationEnd} />')
    ...
    expect(transition).toContain('const languageLabelShown = !modeStage')
    expect(transition).toContain('getInitialBrowserLanguage(language)')
    expect(transition).toContain("setLanguageAffordanceStage('candidate')")
    ...
    expect(transitionCss).toContain('var(--language-candidate-reveal-ms, 220ms)')
    expect(transitionCss).not.toContain('left: calc(50% + 80px)')
    expect(transitionCss).not.toContain('transition-delay: 1900ms')
    ...
    expect(transitionHoverCss).toContain('@keyframes ritual-entry-arrow-draw')
    ...
    expect(transitionHoverCss).toContain("[data-ritual-active='true'] .ritual-hand-affordance__ring")
    expect(transitionHoverCss).toContain('var(--ritual-affordance-width, 96px)')
    expect(transitionHoverCss).toContain('var(--ritual-ring-retract-ms, 300ms)')
    expect(transition).toContain('resolveRitualWheelAction')
    expect(transition).toContain('RITUAL_TIMINGS.RING_RETRACT_MS')
    expect(transition).toContain('data-ritual-wheel-ready')
    expect(transition).toContain('primaryHold.interrupt()')
    ...
    expect(transitionCss).not.toContain('transition-delay: 1250ms')
    expect(transitionCss).toContain('ritual-options-release var(--ritual-content-release-ms, 1600ms)')
  ...
  it('retracts the return affordance before confirmed navigation instead of hiding it in one frame', () => {
    expect(returnControl).toContain('const RETURN_RING_RETRACT_MS = 360')
    expect(returnControl).toContain('const target = ringActive ? 1 : 0')
    expect(returnControl).toContain('pendingCompleteRef.current = true')
    expect(returnControl).toContain('pendingArrowFadeRef.current')
    expect(returnControl).toContain("event.target.classList.contains('reader-return-affordance__arrow')")
    expect(returnControl).toContain("data-return-exit-stage={exitStage}")
    ...
    expect(returnControl).toContain("const affordanceVisible = visualArmed || progress > 0.001 || exitStage !== 'idle'")
    ...
    expect(returnControlCss).toContain('transition: opacity var(--return-arrow-fade-ms, 180ms) ease')
    expect(returnControl).toContain('const RETURN_RING_DRAW_MS = 720')
    expect(returnControl).toContain('}, [ringActive])')
  ...
  it('arms the mobile updates return once and routes the next explicit upward swipe', () => {
    expect(updatesPage).toContain('if (!returnArmed) setReturnArmed(true)')
    expect(updatesPage).toContain('if (!interactive || !returnReady) return undefined')
  ...
  it('draws one return circle and reveals its arrow only after the circle is ready', () => {
    ...
    expect(returnControl).toContain('data-return-ready={arrowVisible')
