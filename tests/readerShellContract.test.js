...
describe('Reader shell contract boundaries', () => {
  ...
  it('draws a compact Landing guide, counter-parallaxes it, and erases it with a paper-colored pass', () => {
    expect(landing).toContain('className="landing-title-n-host"')
    expect(landing).toContain('className="landing-title-n-origin"')
    expect(landing).not.toContain('landing-guide-arrow__seed')
    expect(landing).not.toContain('landing-guide-arrow__wash')
    expect(landing).toContain('className="landing-guide-arrow__strokes"')
    expect(landing).toContain('className="landing-guide-arrow__eraser"')
    expect(landing).toContain('pathLength="1" d={GUIDE_CURVE}')
    expect(landing).toContain('pathLength="1" d={GUIDE_HEAD}')
    expect(landingGuideCss).toContain('left: 0')
    expect(landingGuideCss).toContain('bottom: 50%')
    expect(landingGuideCss).toContain('left: -52px')
    expect(landingGuideCss).toContain('width: 42px')
    expect(landingGuideCss).toContain('height: 34px')
    expect(landingGuideCss).toContain('var(--scene-parallax-back-x, 0px) - var(--scene-parallax-front-x, 0px)')
    expect(landingGuideCss).toContain('var(--scene-parallax-back-y, 0px) - var(--scene-parallax-front-y, 0px)')
    expect(landingGuideCss).toContain('stroke-dasharray: 1')
    expect(landingGuideCss).toContain('@keyframes landing-guide-pencil-curve')
    expect(landingGuideCss).toContain('@keyframes landing-guide-pencil-head')
    expect(landingGuideCss).toContain('stroke: var(--reader-paper')
    expect(landingGuideCss).toContain('@keyframes landing-guide-erase-curve')
  })
  ...
  it('offers the return choice at the real page end and at the final node on direct-input devices', () => {
    ...
    expect(returnControl).toContain("event.pointerType === 'mouse' && hasHoverPointer()) onDisarm()")
