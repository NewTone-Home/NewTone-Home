...
describe('Reader shell contract boundaries', () => {
  ...
  it('blooms the Landing guide from an ink seed at the N origin without line-drawing the arrow', () => {
    expect(landing).toContain('className="landing-title-n-host"')
    expect(landing).toContain('className="landing-title-n-origin"')
    expect(landing).toContain('className="landing-guide-arrow__seed" cx="64" cy="12"')
    expect(landing).toContain('className="landing-guide-arrow__wash"')
    expect(landing).toContain('className="landing-guide-arrow__strokes"')
    expect(landing).toContain('M8 64C13 48 22 39 36 32C47 26 55 20 64 12')
    expect(landing).toContain('M53 15C58 14 62 13 66 10C65 15 64 20 62 24')
    expect(landingCss).toContain('left: .08em')
    expect(landingCss).toContain('bottom: .16em')
    expect(landingCss).toContain('left: -64px')
    expect(landingCss).toContain('@keyframes landing-guide-seed-bloom')
    expect(landingCss).toContain('@keyframes landing-guide-wash-resolve')
    expect(landingCss).toContain('@keyframes landing-guide-strokes-resolve')
  ...
  it('runs Reader return entirely inside the live Landing instead of handing between two logos', () => {
    ...
    expect(landing).toContain('const showSignatureLines = !returnArrival')
    expect(landing).toContain('{showSignatureLines && <NewToneHandLines />}')
